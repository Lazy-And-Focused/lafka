...
class AuthUser implements LAFka.AuthUser {
	...
	public async delete() {
		const auth_user = await this._database.auth_users.delete({profile_id: this._data.profile_id});
		const user = await this._database.users.delete({id: this._data.profile_id});
